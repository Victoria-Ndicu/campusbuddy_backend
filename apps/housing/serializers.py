...
from .models import (
    ...
    RoommatePreference, RoommateProfile, UserHousingPreference,
    LISTING_TAGS, ALERT_RULE_TYPES,
)

VALID_TAGS       = [t[0] for t in LISTING_TAGS]
VALID_RULE_TYPES = [t[0] for t in ALERT_RULE_TYPES]
...
# ---------------------------------------------------------------------------
# User housing preference  (module on/off per user)
# ---------------------------------------------------------------------------

class UserHousingPreferenceSerializer(serializers.ModelSerializer):
    class Meta:
        model  = UserHousingPreference
        fields = ["housing_module_enabled", "updated_at"]
        read_only_fields = ["updated_at"]


# ---------------------------------------------------------------------------
# Housing listing serializers
# ---------------------------------------------------------------------------
...
class CreateHousingListingSerializer(serializers.Serializer):
    ...
    tags           = serializers.ListField(child=serializers.ChoiceField(choices=VALID_TAGS), default=list)
...
class UpdateHousingListingSerializer(serializers.Serializer):
    ...
    tags           = serializers.ListField(child=serializers.ChoiceField(choices=VALID_TAGS), required=False)
...
# ---------------------------------------------------------------------------
# Alert rule serializers  — aligned with Flutter's _AlertRule model
# ---------------------------------------------------------------------------

class HousingAlertSerializer(serializers.ModelSerializer):
    """
    Output shape matches Flutter's _AlertRule.fromJson():
        id, label, rule_type, area, max_price, property_types, is_active
    """
    class Meta:
        model  = HousingAlert
        fields = [
            "id", "rule_type", "label", "area",
            "max_price", "property_types", "is_active",
            "created_at",
        ]
...
class CreateAlertSerializer(serializers.Serializer):
    """Accepts the payload Flutter's HHCreateAlertScreen POSTs."""
    rule_type      = serializers.ChoiceField(choices=VALID_RULE_TYPES, default="new_listing")
    ...
    property_types = serializers.ListField(child=serializers.CharField(), default=list)
    is_active      = serializers.BooleanField(default=True)

    # Legacy fields still accepted for API clients that use the old shape
...
class UpdateAlertSerializer(serializers.Serializer):
    """Accepts Flutter's PATCH body — only is_active is sent when toggling a rule."""
    is_active      = serializers.BooleanField(required=False)
    label          = serializers.CharField(required=False, allow_blank=True)
    area           = serializers.CharField(required=False, allow_blank=True, allow_null=True)
    max_price      = serializers.IntegerField(required=False, allow_null=True)
    property_types = serializers.ListField(child=serializers.CharField(), required=False)
    rule_type      = serializers.ChoiceField(choices=VALID_RULE_TYPES, required=False)


# ---------------------------------------------------------------------------
# Roommate profile serializer  — shaped for Flutter's _Roommate.fromJson
# ---------------------------------------------------------------------------
...
class RoommateProfileSerializer(serializers.ModelSerializer):
    ...
    lifestyle_prefs    = serializers.SerializerMethodField()
    ...
    def get_course(self, obj):
        return getattr(obj.user, 'course', None) or getattr(obj.user, 'program', None) or ''

    def get_year(self, obj):
        year = getattr(obj.user, 'year_of_study', None)
        return f"Year {year} student" if year else ''
    ...
    def get_lifestyle_prefs(self, obj):
        prefs = []
        sleep_labels = {'early_bird': '🌅 Early bird', 'night_owl': '🌙 Night owl', 'flexible': '⏰ Flexible sleep'}
        clean_labels = {'very_clean': '🧹 Very tidy', 'moderate': '🏠 Moderate', 'relaxed': '😌 Relaxed'}
        noise_labels = {'quiet': '🤫 Quiet', 'moderate': '🎵 Moderate', 'lively': '🎉 Social'}
        for val, labels in [(obj.sleep_schedule, sleep_labels), (obj.cleanliness, clean_labels), (obj.noise_level, noise_labels)]:
            if val and val in labels:
                prefs.append(labels[val])
        if not obj.smoking:
...
class CreateRoommateProfileSerializer(serializers.Serializer):
    ...
    sleep_schedule      = serializers.ChoiceField(
                              choices=["early_bird","night_owl","flexible","Early bird","Night owl","Flexible"],
                              required=False)
    cleanliness         = serializers.ChoiceField(
                              choices=["very_clean","moderate","relaxed","Very tidy","Moderate","Relaxed"],
                              required=False)
    noise_level         = serializers.ChoiceField(
                              choices=["quiet","moderate","lively","Quiet","Moderate","Social"],
                              required=False)
...
# ---------------------------------------------------------------------------
# Module settings serializer  (admin)
# ---------------------------------------------------------------------------
